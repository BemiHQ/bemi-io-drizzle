...
export const generateMigration = async (options: { path: string }) => {
  const migrationFilePath = path.join(process.cwd(), options.path);

  fs.writeFileSync(migrationFilePath, `CREATE OR REPLACE FUNCTION _bemi_row_trigger_func()
  RETURNS TRIGGER
AS $$
DECLARE
  _bemi_metadata TEXT;
...
`);
  ...
  console.log(`Migration generated in ${migrationFilePath}`);
};
